#sample generator(normal sample)
#note: this is an updated sample generator, two things are changed, 1. add options to error variance,
#      2. missing function changed to keep all the positve value
...
#       loss_rate(percentage the sample is not missing), method_indicator("exp":use the exp function,
#       "extreme_min": remove miminum y, "extreme_max": remove maximum y)
#output: would be a list, include gamma value, dataset with missing and dataset for simulation(convert to logistic dataset)
...
sample_generator1 <- function(n=100, beta_vector=c(1.5, 2, 3, rep(0,5)), intercept=99,
  ...
  if(method_indicator == "exp"){
    ...
    missing_binary[missing_index] <- 0
  }else stop("method_indicator input should be one of exp, extreme_min, and extreme_max(sample_generator1)")
